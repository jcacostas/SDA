...
#define sendData(args)  (serialPort->write(args))    // Write Over Serial
...
{	
    ...
    while (availableData() > 0){
		...
		if ( (Incoming_Byte == 255) & (peekData() == 255) ){
			...
			if( (Error_Byte = readData()) != 0 )   // Error
				return (Error_Byte*(-1));
			Moving_Byte = readData();         // Moving status
		}
    }
	return (Moving_Byte);               // Returns the read moving status
}
...
{	
    ...
    while (availableData() > 0){
		...
		if ( (Incoming_Byte == 255) & (peekData() == 255) ){
			...
			if( (Error_Byte = readData()) != 0 )   // Error
				return (Error_Byte*(-1));
			RWS_Byte = readData();         // RWS 
		}
    }
	return (RWS_Byte);               // Returns the RWS 
}
...
{	
    ...
    while (availableData() > 0){
		...
		if ( (Incoming_Byte == 255) & (peekData() == 255) ){
			...
			Load_Low_Byte = readData();            // Load Bytes
